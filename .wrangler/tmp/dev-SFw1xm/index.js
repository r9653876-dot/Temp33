...
// .wrangler/tmp/bundle-oT8Vik/checked-fetch.js
...
authRouter.post("/register", async (c) => {
  ...
  try {
    await c.env.DB.batch([
      c.env.DB.prepare("INSERT INTO users (id, email, password_hash, role, status) VALUES (?, ?, ?, ?, ?)").bind(userId, normalizedEmail, hashedPassword, "user", "active"),
      c.env.DB.prepare("INSERT INTO profiles (id, user_id, full_name, date_of_birth, location, bio, interests, relationship_preference, profile_photo_url, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)").bind(profileId, userId, full_name, date_of_birth, location, bio, JSON.stringify(interests || []), relationship_preference, profile_photo_url, "pending")
    ]);
...
authRouter.post("/login", async (c) => {
  ...
  const expiresDate = new Date(Date.now() + 7 * 24 * 60 * 60 * 1e3);
  const expiresAtSQLite = expiresDate.toISOString().replace("T", " ").split(".")[0];
  await c.env.DB.prepare("INSERT INTO sessions (id, user_id, session_token_hash, expires_at) VALUES (?, ?, ?, ?)").bind(sessionId, user.id, tokenHash, expiresAtSQLite).run();
...
var uuidv42 = /* @__PURE__ */ __name(() => crypto.randomUUID(), "uuidv4");
adminRouter.post("/login", async (c) => {
  ...
  let admin;
  try {
    admin = await c.env.DB.prepare("SELECT id, password_hash FROM admin_users WHERE email = ?").bind(normalizedEmail).first();
  ...
  try {
    ...
    const sessionToken = generateSessionToken();
    const tokenHash = await hashSessionToken(sessionToken);
    const expiresDate = new Date(Date.now() + 2 * 24 * 60 * 60 * 1e3);
    const expiresAtSQLite = expiresDate.toISOString().replace("T", " ").split(".")[0];
    await c.env.DB.prepare("INSERT INTO admin_sessions (id, admin_id, session_token_hash, expires_at) VALUES (?, ?, ?, ?)").bind(sessionId, admin.id, tokenHash, expiresAtSQLite).run();
...
adminRouter.get("/stats", async (c) => {
  try {
    ...
    return c.json({
      ...
      pendingProfiles: pending.results[0].count,
      approvedProfiles: approved.results[0].count,
      rejectedProfiles: rejected.results[0].count,
      suspendedProfiles: suspended.results[0].count
    });
  } catch (e) {
...
async function updateProfileStatus(c, userId, newStatus) {
  ...
  await c.env.DB.batch([
    ...
    c.env.DB.prepare("INSERT INTO profile_status_history (id, profile_id, admin_id, old_status, new_status, reason) VALUES (?, ?, ?, ?, ?, ?)").bind(historyId, profile.id, adminId, oldStatus, newStatus, reason)
  ]);
  return c.json({ message: `Profile status updated to ${newStatus}` });
}
__name(updateProfileStatus, "updateProfileStatus");
...
// .wrangler/tmp/bundle-oT8Vik/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-oT8Vik/middleware-loader.entry.ts
